...
def get_common_ids(a_seq, b_seq, min_match_size=3):
    """
    Find common intervals between two sequences.

    Args:
        a_seq: First sequence.
        b_seq: Second sequence.
        min_match_size: Minimum size of matching block considered valid.

    Returns:
        Tuple of two lists: (common_a, common_b), each containing the indices of common tokens.
    """
    a_matches, b_matches = get_match_info(a_seq, b_seq, min_match_size=min_match_size)

    # Convert matches to ids
    common_a = spans2ids(a_matches)
    common_b = spans2ids(b_matches)

    return sorted(set(common_a)), sorted(set(common_b))
...
if __name__ == '__main__':
    ...
    # seq1 = text1
    # seq2 = text2
    color_print_diff_pair(seq1, seq2, min_word_match, sep=' ', use_split=False)
